...
export default function PlayCompleted({
  ...
  const [playerMode, setPlayerMode] = useState<"embed" | "builtin">("builtin");
  ...
  const playerModeRef = useRef<"embed" | "builtin">("builtin");
  ...
  const switchToTrack = useCallback(
    async (nextIndex: number) => {
      ...
      const useEmbed = preferEmbedRef.current && mpvAvailableRef.current && !htmlAudio;
  ...
  const playBuiltinMedia = useCallback(
    async (el: HTMLMediaElement, onFail: () => void) => {
      setError("");
      resetPlaybackEnded();
      el.load();
      try {
  ...
  useEffect(() => {
    ...
    if (currentUsesHtmlAudio) {
      ...
      if (!el) return;
      void playBuiltinMedia(el, () => void handleAudioError());
      return;
    }
    ...
    if (!el) return;
    void playBuiltinMedia(el, () => void handleVideoError());
  }, [
    mediaSrc,
