...
#[cfg(windows)]
fn augment_path_for_gui() {
    use std::path::PathBuf;

    let mut extras = Vec::new();
    if let Ok(local) = std::env::var("LOCALAPPDATA") {
        extras.push(PathBuf::from(&local).join("Microsoft/WinGet/Links"));
    }
    if let Ok(pf) = std::env::var("ProgramFiles") {
        extras.push(PathBuf::from(&pf).join("yt-dlp"));
    }
    ...
    let mut merged: Vec<PathBuf> = extras;
